...
function CTAButtons({ slug }: { slug: string }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 mb-6">
      <Link href={`/fr/contact?service=${slug}`} className="bg-orange-600 text-white font-bold px-8 py-4 rounded-2xl hover:bg-orange-700 text-center shadow-lg">
        Démo Sans Engagement
      </Link>
      {showPhone ? (
        <a href={`tel:${CONTACT.phone}`} className="border-2 border-orange-600 text-orange-600 font-bold px-8 py-4 rounded-2xl hover:bg-orange-600 hover:text-white transition-all text-center">
          {CONTACT.phoneDisplay}
        </a>
      ) : (
        <a href={CONTACT.whatsapp} target="_blank" rel="noopener noreferrer" className="border-2 border-orange-600 text-orange-600 font-bold px-8 py-4 rounded-2xl hover:bg-orange-600 hover:text-white transition-all text-center">
          💬 WhatsApp 24/7
        </a>
      )}
    </div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        <div className="w-full lg:w-1/2 animate-slide-left">
          ...
          <h1 className="text-4xl lg:text-5xl font-black mb-6 leading-tight text-slate-900">
            CRM Intégré &<br/>
            <span className="bg-gradient-to-r from-orange-600 to-orange-700 bg-clip-text text-transparent">Listes de Prospects</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            {['SuiteCRM inclus', 'Listes B2B/B2C', 'Mautic intégré', 'Rapports auto'].map((b, i) => (
              <span key={b} className={`flex items-center gap-2 bg-white text-slate-700 text-sm font-medium px-4 py-2 rounded-full shadow-md animate-fade-in-up animate-delay-${(i+1)*100}`}><CheckIcon className="w-5 h-5 text-orange-600" /> {b}</span>
            ))}
          </div>
        </div>
        <div className="w-full lg:w-1/2 animate-slide-right">
          <div className="relative">
            <div className="absolute -inset-4 bg-gradient-to-r from-orange-400 to-orange-500 rounded-3xl blur-2xl opacity-20"></div>
...
<section className="bg-gradient-to-br from-slate-900 via-orange-950 to-orange-900 text-white py-20 lg:py-28 overflow-hidden relative">
<div className="absolute inset-0 pointer-events-none">
<div className="absolute -top-32 -right-32 w-[500px] h-[500px] bg-orange-500 opacity-10 rounded-full blur-3xl"></div>
</div>
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
<div className="text-center mb-12">
<h2 className="text-3xl lg:text-4xl font-black mb-4">Ce qui est inclus</h2>
<p className="text-orange-200 text-lg max-w-2xl mx-auto">Tout ce dont vous avez besoin pour gérer vos contacts et vos ventes.</p>
</div>
...
<div><p className="text-4xl font-black text-orange-600">250K+</p><p className="text-slate-500 text-sm mt-1">Contacts B2B</p></div>
<div><p className="text-4xl font-black text-orange-600">98%</p><p className="text-slate-500 text-sm mt-1">Données validées</p></div>
<div><p className="text-4xl font-black text-orange-600">+35%</p><p className="text-slate-500 text-sm mt-1">Taux conversion</p></div>
<div><p className="text-4xl font-black text-orange-600">48h</p><p className="text-slate-500 text-sm mt-1">Setup complet</p></div>
        </div>
      </section>
...
<div className="w-12 h-12 bg-orange-600 text-white rounded-xl flex items-center justify-center font-black text-xl flex-shrink-0 shadow-md">{n}</div>
              <div className="pt-1">
...
<li className="flex items-center gap-3 text-slate-700"><span className="w-6 h-6 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center"><CheckIcon className="w-4 h-4"/></span>Formation incluse — pas de courbes d'apprentissage interminables</li>
				<li className="flex items-center gap-3 text-slate-700"><span className="w-6 h-6 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center"><CheckIcon className="w-4 h-4"/></span>Support francophone basé au Québec</li>
				<li className="flex items-center gap-3 text-slate-700"><span className="w-6 h-6 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center"><CheckIcon className="w-4 h-4"/></span>Mises à jour automatiques, sans frais cachés</li>
				<li className="flex items-center gap-3 text-slate-700"><span className="w-6 h-6 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center"><CheckIcon className="w-4 h-4"/></span>Intégré à nos services d'appels sortants</li>
              </ul>
              <Link href="/fr/contact?service=crm" className="inline-flex items-center gap-2 bg-orange-600 text-white px-6 py-3.5 rounded-xl font-bold hover:bg-orange-700 transition-colors">
                Voir une démo →
              </Link>
            </div>
...
<div className="w-16 h-1 bg-orange-600 mx-auto rounded"></div>
            ...
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-700 rounded-full flex items-center justify-center font-bold text-white text-sm">{t.av}</div>
                  <div>
      ...
      <section className="bg-gradient-to-br from-slate-900 to-orange-800 py-20">
        <div className="max-w-4xl mx-auto px-4 text-center text-white">
          <h2 className="text-3xl lg:text-4xl font-black mb-4">Prêt à centraliser vos leads?</h2>
          <p className="text-orange-200 text-lg mb-10 max-w-2xl mx-auto">CRM + listes qualifiées + support francophone. Tout ce qu\'il vous faut pour vendre plus.</p>
          <CTAButtons slug="crm"/>
      ...
      <section className="bg-white py-20 border-t border-slate-100">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <Link href="/fr/contact?service=crm" className="inline-block mt-8 bg-orange-600 text-white font-bold px-8 py-4 rounded-xl hover:bg-orange-700">
            Démarrer Maintenant
          </Link>
        </div>
      </section>
