      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mb-5 leading-relaxed">
              {fr
                ? "Centre d’appels & IA pour PME. Conseillers professionnels et agents IA vocaux 24/7."
                : "Call center & AI for SMBs. Professional agents and AI voice agents 24/7."}
            </p>
            <div className="flex gap-3">
              <a href={CONTACT.whatsapp} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 bg-green-700 hover:bg-green-500 rounded-lg flex items-center justify-center transition-colors">
                ...
                <a href={CONTACT.calendly} target="_blank" rel="noopener noreferrer"
                  ...
                  <div>
                    <p className="text-white text-sm font-semibold group-hover:text-blue-300 transition-colors">
                      {fr ? 'Réserver 30 min' : 'Book 30 min'}
                    </p>
        ...
        <div className="border-t border-slate-800 pt-8 mb-6">
          <div className="flex flex-wrap gap-3 justify-center">
            {['🔒 RGPD Conforme', '🇨🇦 Montréal, Canada', '⭐ 98% Satisfaction', '⚡ 48h Setup', '24/7 Disponible'].map(b => (
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500">
          ...
          <div className="flex gap-5">
            <Link href={fr ? '/fr/confidentialite' : '/en/privacy'} className="hover:text-white transition-colors">
              {fr ? 'Confidentialité' : 'Privacy'}
            </Link>
            <Link href={fr ? '/fr/cgv' : '/en/terms'} className="hover:text-white transition-colors">
              {fr ? 'CGV' : 'Terms'}
            </Link>
            <Link href={fr ? '/en' : '/fr'} className="hover:text-white transition-colors">
              {fr ? 'English' : 'Français'}
            </Link>
          </div>
        </div>
